...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        std::string strUsage = _("Dash Platinum Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  dashpd [options]                     " + _("Start Dash Platinum Core Daemon") + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "dashp:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in dashd anymore. Use the dashp-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Dash Platinum Core server starting\n");
